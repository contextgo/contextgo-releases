...
<div className='editorial-note'>
  ...
  <p>
    The real problem is not whether a model can answer. It is whether an agent can work inside real projects for a long
...
1. bring the work reality you already use into ContextGo
2. send the agent's results back into that same work reality
...
1. _Why ContextGo starts from context before agents_
   Understand why we place the context layer at the center of the product.
2. _Desktop host, mobile client: keep the product model explicit_
   Understand why remote access only works when the Host / Client boundary stays clear.
3. _Release operations should flow through contextgo-releases_
